...
class AbstractLeague {
    ...
    function getLeagueRaces($type='') {
        $SQL = $this->wpdb->prepare("select l.ID as lid,
            l.post_title,
			e.ID as eid,e.post_title as etitle,LEFT(e.post_title,8) as etag,e.post_date as edate, e.guid as eurl,
			r.ID as rid,r.post_title as rtitle,r.post_name as rname,
			r_type.meta_value as rtype
			from wp_posts l
